...
1. QGIS → Модули → Управление модулями → найди «LazyGeo» → Установить
   (плагин опубликован в [репозитории QGIS](https://plugins.qgis.org/plugins/LazyGeo/), zip-файл можно также скачать там вручную или взять из [Releases на GitHub](https://github.com/CatSprite-dev/lazyGeo/releases))
2. Открой плагин через Модули → LazyGeo
3. Выбери Excel → при необходимости поправь название слоя → Запустить
...
1. QGIS → Plugins → Manage and Install Plugins → search "LazyGeo" → Install
   (also published on the [QGIS plugin repository](https://plugins.qgis.org/plugins/LazyGeo/), where the zip can be downloaded manually, or grab it from [GitHub Releases](https://github.com/CatSprite-dev/lazyGeo/releases))
2. Open the plugin via Plugins → LazyGeo
3. Select Excel → adjust layer name if needed → Run
